...
with open('pre_processed_dataset/data_set_k48_B50_z2.npy', 'rb') as f:
    data_set = np.load(f)
    #Test_set_down = np.reshape(Test_set_down, (Test_set_down.shape[0], Test_set_down.shape[1], x_sz, y_sz))
    
with open('pre_processed_dataset/labels_k48_B50_z2.npy', 'rb') as f:
...
event_num = []
for k in range(K):
    model = torch.load("saved_models/mini_eCNN_" + str(k) + ".pt")
